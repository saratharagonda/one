...
<head>
  ...
  <title>SocialConnect · Facebook style</title>
  <!-- Font Awesome (free) -->
  ...
  <style>
    * {
    ...
    .sidebar-card .user-row:hover {
      background: var(--fb-bg);
    }
    .sidebar-card .user-row i {
      width: 28px;
      color: var(--fb-blue);
      font-size: 18px;
      text-align: center;
    ...
    <main class="feed">
      ...
      <div class="composer">
        ...
        <div class="composer-actions">
          ...
          <button class="photo"><i class="fas fa-images"></i> Photo/Video</button>
          <button class="feel"><i class="fas fa-smile"></i> Feeling</button>
        </div>
      </div>

      <!-- feed posts (static + dynamic) -->
      <div id="feedContainer">
        <!-- post 1 -->
        <div class="post">
          <div class="post-header">
            <div class="avatar-small"><img src="https://i.pravatar.cc/44?img=8" /></div>
            <div><div class="post-user">David Chen</div><div class="post-time">2h ago · 🌍</div></div>
          </div>
          <div class="post-content"><p>Just finished a 10k run! 🏃‍♂️ feeling great. Anyone else training for the marathon?</p></div>
          <div class="post-image"><img src="https://images.unsplash.com/photo-1534432588723-1b9d5e09e7d9?w=600&h=300&fit=crop" alt="running" /></div>
          <div class="post-stats"><span>❤️ 24</span><span>12 comments · 5 shares</span></div>
          <div class="post-actions"><button><i class="far fa-thumbs-up"></i> Like</button><button><i class="far fa-comment"></i> Comment</button><button><i class="fas fa-share"></i> Share</button></div>
        </div>

        <!-- post 2 -->
        <div class="post">
          <div class="post-header">
            <div class="avatar-small"><img src="https://i.pravatar.cc/44?img=12" /></div>
            <div><div class="post-user">Mia Rodriguez</div><div class="post-time">5h ago · 📷</div></div>
          </div>
          <div class="post-content"><p>Sunset at the pier 🌅 grateful for this view.</p></div>
          <div class="post-image"><img src="https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&h=300&fit=crop" alt="sunset" /></div>
          <div class="post-stats"><span>❤️ 43</span><span>8 comments · 2 shares</span></div>
          <div class="post-actions"><button><i class="far fa-thumbs-up"></i> Like</button><button><i class="far fa-comment"></i> Comment</button><button><i class="fas fa-share"></i> Share</button></div>
        </div>
      </div>
    </main>
  ...
  <script>
    (function() {
      ...
      function addNewPost(text) {
        ...
        postDiv.innerHTML = `
          <div class="post-header">
            <div class="avatar-small"><img src="https://i.pravatar.cc/44?img=11" /></div>
            <div><div class="post-user">Elena Park</div><div class="post-time">Just now · 🌐</div></div>
          </div>
          <div class="post-content"><p>${escapeHtml(text)}</p></div>
          <div class="post-stats"><span>❤️ 0</span><span>0 comments</span></div>
          <div class="post-actions"><button><i class="far fa-thumbs-up"></i> Like</button><button><i class="far fa-comment"></i> Comment</button><button><i class="fas fa-share"></i> Share</button></div>
        `;
        feedContainer.prepend(postDiv);
        // animate
        postDiv.style.transition = '0.2s';
        postDiv.style.opacity = '0.6';
        setTimeout(() => postDiv.style.opacity = '1', 50);
        // attach like listener
        attachLikeListeners(postDiv);
        // clear input
        postInput.value = '';
      }
      ...
      function attachLikeListeners(container) {
        const likeBtns = container.querySelectorAll('.post-actions button:first-child');
      ...
      // also bind photo button (demo)
      document.querySelector('.composer-actions .photo')?.addEventListener('click', function() {
        const dummyText = "📸 Check out this awesome photo!";
        addNewPost(dummyText);
      });

      // attach like to existing posts
      document.querySelectorAll('.post').forEach(p => attachLikeListeners(p));

      // global search (demo)
      document.getElementById('globalSearch')?.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
          alert('Search for: "' + this.value + '" (demo)');
        }
      });
